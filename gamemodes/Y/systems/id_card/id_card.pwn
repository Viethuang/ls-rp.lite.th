...
{
    LoadID_Card(playerid, false);
    DeletePVar(playerid, "ShowPlayerIDCard");
    return 1;
}


stock LoadID_Card(playerid, bool:show = false)
{
    if(show)
    {
        ...
        PlayerTextDrawSetPreviewModel(playerid, PlayerID_Card[playerid][2], PlayerInfo[playerid][pLastSkin]);
        ...
        format(str, sizeof(str), "ID: %d", PlayerInfo[playerid][pId_number]);
		PlayerTextDrawSetString(playerid, PlayerID_Card[playerid][6], str);

        format(str, sizeof(str), "Name: %s", ReturnName(playerid));
		PlayerTextDrawSetString(playerid, PlayerID_Card[playerid][8], str);

        format(str, sizeof(str), "DOB: %s", PlayerInfo[playerid][pId_dob]);
		PlayerTextDrawSetString(playerid, PlayerID_Card[playerid][9], str);

        format(str, sizeof(str), "GENDER: %s", PlayerInfo[playerid][pId_gebder]);
		PlayerTextDrawSetString(playerid, PlayerID_Card[playerid][11], str);

        format(str, sizeof(str), "EXP: ~r~%s", PlayerInfo[playerid][pid_expire]);
		PlayerTextDrawSetString(playerid, PlayerID_Card[playerid][11], str);


        for(new i = 0; i < 12; i++)
        {
            PlayerTextDrawShow(playerid, PlayerID_Card[playerid][i]);
        }

        SetPVarInt(playerid, "ShowPlayerIDCard", 1);
        return 1;
    }
    else
    {
        for(new i = 0; i < 12; i++)
        {
            PlayerTextDrawDestroy(playerid, PlayerID_Card[playerid][i]);
        }
        DeletePVar(playerid, "ShowPlayerIDCard");
    }
...
{
    if(!GetPVarInt(playerid, "ShowPlayerIDCard"))
        LoadID_Card(playerid, true);
    else
        LoadID_Card(playerid, false);
